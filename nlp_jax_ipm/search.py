...
from search_methods import decrease_infeasibility


def backtracking_line_search(cost_function, weights_0, slacks_0, lagrange_multipliers_0, search_direction, alpha_smax,
                             alpha_lmax, num_weights, barrier_val, merit_function_parameter, minimal_step,
                             equality_constraints=None, inequality_constraints=None, num_equality_constraints=0,
                             num_inequality_constraints=0, verbosity=1, backtracking_line_search_parameter=0.995,
                             armijo_val=1.0E-4):
    """
    ...
    optimization_return_signal, weights, slacks, merit_function_result, gradient_merit_function_result, correction = \
        setup_search(cost_function, weights_0, slacks_0, merit_function_parameter, barrier_val, equality_constraints,
                     inequality_constraints, num_weights, num_equality_constraints, num_inequality_constraints,
    ...
    if num_inequality_constraints:
        ...
        if new_merit_val > old_merit_val:
            ...
            if jnp.sum(jnp.abs(correction_new)) > jnp.sum(jnp.abs(correction_old)):
                ...
                feasibility_restoration_direction, alpha_corr, correction = \
                    decrease_infeasibility(weights, num_weights, equality_constraints,
                                           inequality_constraints, num_equality_constraints,
                                           num_inequality_constraints, correction_new, cost_function,
                                           weights_0, slacks_0, alpha_smax, search_direction_weights,
                                           search_direction_slacks, merit_function_parameter, barrier_val,
                                           correction, merit_function_result, armijo_val,
                                           gradient_merit_function_result, weight_precision_tolerance,
                                           verbosity, backtracking_line_search_parameter)
            if not correction:
        ...
        if merit_function(cost_function, weights_0 + alpha_smax * search_direction_weights, slacks_0,
            ...
            if num_equality_constraints:
                ...
                if jnp.sum(jnp.abs(correction_new)) > jnp.sum(jnp.abs(correction_old)):
                    feasibility_restoration_direction, alpha_corr, correction = \
                        decrease_infeasibility(weights, num_weights, equality_constraints, inequality_constraints,
                                               ...
                                               search_direction_slacks, merit_function_parameter, barrier_val,
                                               correction, merit_function_result, armijo_val,
                                               gradient_merit_function_result, weight_precision_tolerance, verbosity,
                                               backtracking_line_search_parameter)
...
def setup_search(cost_function, weights_0, slacks_0, merit_function_parameter, barrier_val, equality_constraints,
                 inequality_constraints, num_weights, num_equality_constraints, num_inequality_constraints,
                 search_direction, minimal_step):
    """
    Extract weights, slacks, and multipliers, calculate initial merit result and gradient merit result and initialize
    optimization signal and correction.
    """
    weights = weights_0  # left np.copy away
    slacks = slacks_0
    merit_function_result = \
        merit_function(cost_function, weights_0, slacks_0, merit_function_parameter, barrier_val, equality_constraints,
                       inequality_constraints, num_equality_constraints, num_inequality_constraints)
    ...
    gradient_merit_function_result = \
        gradient_merit_function(cost_function, weights_0, slacks_0,
                                ...
                                num_weights, barrier_val, minimal_step, equality_constraints, inequality_constraints,
                                num_equality_constraints, num_inequality_constraints, merit_function_parameter)
    optimization_return_signal = 0
    correction = False

    return optimization_return_signal, weights, slacks, merit_function_result, gradient_merit_function_result, \
        correction
