...
def solver(equality_constraints, inequality_constraints, weights, slacks=None, lagrange_multipliers=None):
    ...
    if number_equality_constraints or number_inequality_constraints:
        if input_lagrange_multipliers is None:
            lagrange_multipliers = init_lagrange_multipliers(weights, number_weights, number_equality_constraints, number_inequality_constraints, equality_constraints, inequality_constraints)
            if number_inequality_constraints and number_equality_constraints:
                ...
                lagrange_multipliers_inequality[lagrange_multipliers_inequality < np.float64(0.0)] = np.float64(kkt_tolerance)
    ...
    kkt_weights, kkt_slacks, kkt_equality_lagrange_multipliers, kkt_inequality_lagrange_multipliers = KKT(weights, slacks, lagrange_multipliers)
    ...
    for outer_iteration_variable in number_outer_iterations: # adjusting barrier parameter
        ...
        if all([np.linalg.norm(kkt_weights) <= kkt_tolerance, np.linalg.norm(kkt_slacks) <= kkt_tolerance,
                np.linalg.norm(kkt_equality_lagrange_multipliers) <= kkt_tolerance, np.linalg.norm(kkt_inequality_lagrange_multipliers) <= kkt_tolerance]):
        ...
        for inner_iteration_variable in number_inner_iterations:
            ...
            if all([np.linalg.norm(kkt_weights) <= barrier_tolerance, np.linalg.norm(kkt_slacks) <= barrier_tolerance,
                    np.linalg.norm(kkt_equality_lagrange_multipliers) <= barrier_tolerance, np.linalg.norm(kkt_inequality_lagrange_multipliers) <= barrier_tolerance]):
            ...
            hessian = regularize_hessian(hessian(objective_function)(weights, slacks, lagrange_multipliers)) # TODO: nachprüfen, ob das so klappt
            # calculate search_direction
            search_direction = jnp.linalg.solve(hessian_matrix, gradient.reshape((gradient.size, 1))).reshape((gradient.size,))  # reshape gradient and result
            ...
            if number_inequality_constraints or number_equality_constraints:
                ...
                search_direction[number_weights + number_inequality_constraints:] = -search_direction[number_weights + number_inequality_constraints:]

            if number_inequality_constraints or number_equality_constraints:
                # update the merit function parameter, if necessary
                merit_threshold = np.dot(barrier_cost_gradient(objective_function, weights, slacks, number_inequality_constraints), search_direction[:number_weights + number_inequality_constraints]) / (1 - update_factor_merit_function_parameter) / np.sum(np.abs(concatenate_constraints(weights, slacks, equality_constraints, inequality_constraints, number_equality_constraints, number_inequality_constraints)))
            ...
            if number_inequality_constraints:
                ...
                alpha_smax = step(slacks, search_direction[number_weights:(number_weights + number_inequality_constraints)], weight_precision_tolerance, backtracking_line_search_parameter)
                alpha_lmax = step(lagrange_multipliers[number_equality_constraints:], search_direction[(number_weights + number_inequality_constraints + number_equality_constraints):], weight_precision_tolerance, backtracking_line_search_parameter)
                # use a backtracking line search to update weights, slacks, and multipliers
                weights, slacks, lagrange_multipliers, opzimization_return_signal = search(weights, slacks, lagrange_multipliers, search_direction, np.float64(alpha_smax), np.float64(alpha_lmax), number_weights, equality_constraints, inequality_constraints, number_equality_constraints, number_inequality_constraints, verbosity, backtracking_line_search_parameter, armijo_val)
            else:
                # use a backtracking line search to update weights, slacks, and multipliers
                weights, slacks, lagrange_multipliers, opzimization_return_signal = search(weights, slacks, lagrange_multipliers, search_direction, np.float64(1.0), np.float64(1.0), number_weights, equality_constraints, inequality_constraints, number_equality_constraints, number_inequality_constraints, verbosity, backtracking_line_search_parameter, armijo_val)

            kkt_weights, kkt_slacks, kkt_equality_lagrange_multipliers, kkt_inequality_lagrange_multipliers = KKT(objective_function, weights, slacks, lagrange_multipliers, number_inequality_constraints, number_equality_constraints)  # calculate the updated KKT conditions

            if all([convergence_tolerance_objective_function is not None, not number_inequality_constraints, opzimization_return_signal != -2]):
                ...
                if np.abs(old_objective_function - new_objective_function) <= np.abs(convergence_tolerance_objective_function):
        ...
        if all([convergence_tolerance_objective_function is not None, number_inequality_constraints, opzimization_return_signal != -2]):
        ...
        if number_inequality_constraints:
            ...
            update_value = number_inequality_constraints * np.min(slacks * lagrange_multipliers[number_equality_constraints:]) / (np.dot(slacks, lagrange_multipliers[number_equality_constraints:]) + minimal_step)
            # calculation: Nocedal & Wright 19.20
            barrier_val = (0.1 * np.min([0.05 * (1.0 - update_value) / (update_value + minimal_step), 2.0]) ** 3 * np.dot(slacks, lagrange_multipliers[number_equality_constraints:]) / number_inequality_constraints)
...
def init_lagrange_multipliers(weights, number_weights, number_equality_constraints=0, number_inequality_constraints=0, equality_constraints=None, inequality_constraints=None):
    # TODO: eliminate comments
    # if number_equality_constraints or number_inequality_constraints:
        return np.dot(np.linalg.pinv(jacobian_objective_function(weights, number_weights, equality_constraints, inequality_constraints, number_equality_constraints, number_inequality_constraints)[:number_weights, :]), gradient_values(objective_function, 0)(weights) .reshape((number_weights, 1))).reshape((number_equality_constraints + number_inequality_constraints,))
...
def KKT(objective_function, weights, slacks, lagrange_multipliers, number_inequality_constraints, number_equality_constraints):
...
def concatenate_constraints(weights, slacks, equality_constraints=None, inequality_constraints=None, number_equality_constraints=0, number_inequality_constraints=0):
    if number_equality_constraints and number_inequality_constraints:
        return np.concatenate([equality_constraints(weights).reshape((number_equality_constraints,)), (inequality_constraints(weights) - slacks).reshape((number_inequality_constraints,))], axis=0)
    elif number_equality_constraints:
...
def merit_function(objective_function, weights, slacks, merit_function_parameter, equality_constraints=None, inequality_constraints=None, number_equality_constraints=0, number_inequality_constraints=0):
    if number_equality_constraints and number_inequality_constraints:
        return (objective_function(weights) + merit_function_parameter *  (np.sum(np.abs(equality_constraints(weights))) + np.sum(np.abs(inequality_constraints(weights) - slacks))) - barrier_function(slacks))
    elif number_equality_constraints:
        return objective_function(weights) + merit_function_parameter * np.sum(np.abs(equality_constraints(weights)))
    elif number_inequality_constraints:
        return objective_function(weights) + merit_function_parameter * np.sum(np.abs(inequality_constraints(weights) - slacks)) - barrier_function(slacks)
    else:
        return objective_function(weights)


def jacobian_objective_function(weights, number_weights, equality_constraints=None, inequality_constraints=None, number_equality_constraints=0, number_inequality_constraints=0):
    # all reshapes necessary? TODO
    if number_equality_constraints and number_inequality_constraints:
        # TODO: jacfwd einseitig?
        jacobian_top = np.concatenate([jax.jacfwd(equality_constraints)(weights).reshape(number_equality_constraints, number_weights).reshape(number_weights, number_equality_constraints).reshape((number_weights, number_equality_constraints)), jax.jacfwd(inequality_constraints)(weights).reshape(number_inequality_constraints, number_weights).reshape(number_weights, number_inequality_constraints).reshape((number_weights, number_inequality_constraints))], axis=1)
        jacobian_bottom = np.concatenate([np.zeros((number_inequality_constraints, number_equality_constraints)), -np.eye(number_inequality_constraints)], axis=1)
        return np.concatenate([jacobian_top, jacobian_bottom], axis=0)
    elif number_equality_constraints:
        return jax.jacfwd(equality_constraints)(weights).reshape(number_equality_constraints, number_weights).reshape(number_weights, number_equality_constraints).reshape((number_weights, number_equality_constraints))
    elif number_inequality_constraints:
        return np.concatenate([jax.jacfwd(inequality_constraints)(weights).reshape(number_inequality_constraints, number_weights).reshape(number_weights, number_inequality_constraints).reshape((number_weights, number_inequality_constraints)), -np.eye(number_inequality_constraints)], axis=0)
    else:
        return np.zeros((number_weights + number_inequality_constraints, number_equality_constraints + number_inequality_constraints))


def gradient_merit_function(objective_function, weights, slacks, number_weights, equality_constraints=None, inequality_constraints=None, number_equality_constraints=0, number_inequality_constraints=0, barrier_initialization_parameter=0.2, merit_function_parameter=10.0):
    if number_equality_constraints and number_inequality_constraints:
        # derivative of objective function
        # before: gradient(objective_function(weights))
        return (np.dot(grad(objective_function)(weights), search_direction[:number_weights]) - merit_function_parameter * (np.sum(np.abs(equality_constraints(weights))) + np.sum(np.abs(inequality_constraints(weights) - slacks))) - np.dot(barrier_initialization_parameter / (slacks + minimal_step), search_direction[number_weights:]))
    elif number_equality_constraints:   # TODO: nachprüfen, ob das so klappt
        return (np.dot(grad(objective_function)(weights), search_direction[:number_weights]) - merit_function_parameter * np.sum(np.abs(equality_constraints(weights))))
    elif number_inequality_constraints:
        return (np.dot(grad(objective_function)(weights), search_direction[:number_weights]) - merit_function_parameter * np.sum(np.abs(inequality_constraints(weights) - slacks)) - np.dot(barrier_initialization_parameter / (slacks + minimal_step), search_direction[number_weights:]))
    else:
...
def barrier_cost_gradient(objective_function, weights, slacks, number_inequality_constraints):
    if number_inequality_constraints:
        return np.concatenate([gradient_values(objective_function, 0)(weights), -barrier_val / (slacks + minimal_value)], axis=0)
    else:
        return gradient_values(objective_function, 0)(weights)  # TODO: to be revisited


def search(weights_0, slacks_0, lagrange_multipliers_0, search_direction, alpha_smax, alpha_lmax, number_weights, equality_constraints=None, inequality_constraints=None, number_equality_constraints=0, number_inequality_constraints=0, verbosity=1, backtracking_line_search_parameter=0.995, armijo_val = 1.0E-4):
    ...
    gradient_merit_function_result = gradient_merit_function(weights_0, slacks_0, search_direction[:number_weights + number_inequality_constraints])
    correction = False
    if number_inequality_constraints:
        # step search when there are inequality constraints
        if merit_function(objective_function, weights_0 + alpha_smax * search_direction_weights, slacks_0 + alpha_smax * search_direction_slacks, merit_function_parameter) > merit_function_result + alpha_smax * armijo_val * gradient_merit_function_result:
            # second-order correction
            correction_old = concatenate_constraints(weights_0, slacks_0, equality_constraints, inequality_constraints, number_equality_constraints, number_inequality_constraints)
            correction_new = concatenate_constraints(weights_0 + alpha_smax * search_direction_weights, slacks_0 + alpha_smax * search_direction_slacks, equality_constraints, inequality_constraints, number_equality_constraints, number_inequality_constraints)
            if np.sum(np.abs(correction_new)) > np.sum(np.abs(correction_old)):
                # infeasibility has increased, attempt to correct
                jacobian_matrix_objective_function = jacobian_objective_function(weights, number_weights, equality_constraints, inequality_constraints, number_equality_constraints, number_inequality_constraints)
                try:
                    feasibility_restoration_direction = -jnp.linalg.solve(jacobian_matrix_objective_function, correction_new.reshape((number_weights + number_inequality_constraints, 1))).reshape((number_weights + number_inequality_constraints,))
                except:
                    # if the Jacobian is not invertible, find the minimum norm solution instead
                    feasibility_restoration_direction = -np.linalg.lstsq(jacobian_matrix_objective_function, correction_new, rcond=None)[0]  # jnp instead of np
                if (merit_function(objective_function, weights_0 + alpha_smax * search_direction_weights + feasibility_restoration_direction[:number_weights], slacks_0 + alpha_smax * search_direction_slacks + feasibility_restoration_direction[number_weights:], merit_function_parameter) <= merit_function_result + alpha_smax * armijo_val * gradient_merit_function_result):
                    alpha_corr = step(slacks_0, alpha_smax * search_direction_slacks + feasibility_restoration_direction[number_weights:])
                    if (merit_function(objective_function, weights_0 + alpha_corr * (alpha_smax * search_direction_weights + feasibility_restoration_direction[:number_weights]), slacks_0 + alpha_corr * (alpha_smax * search_direction_slacks + feasibility_restoration_direction[number_weights:], merit_function_parameter)) <= merit_function_result + alpha_smax * armijo_val * gradient_merit_function_result):
                        if verbosity > 2:
            ...
            if not correction:
                ...
                while merit_function(objective_function, weights_0 + alpha_smax * search_direction_weights, slacks_0 + alpha_smax * search_direction_slacks, merit_function_parameter) > merit_function_result + alpha_smax * armijo_val * gradient_merit_function_result:
                    # backtracking line search
                    if (np.sqrt(np.linalg.norm(alpha_smax * search_direction_weights) ** 2 + np.linalg.norm(alpha_lmax * search_direction_slacks) ** 2) < minimal_step):
        ...
        if correction:
            slacks = slacks_0 + alpha_corr * (alpha_smax * search_direction_slacks + feasibility_restoration_direction[number_weights:])
        else:
            slacks = slacks_0 + alpha_smax * search_direction_slacks
    else:
        # step search for only equality constraints or unconstrained problems
        if merit_function(objective_function, weights_0 + alpha_smax * search_direction_weights, slacks_0, merit_function_parameter) > merit_function_result + alpha_smax * armijo_val * gradient_merit_function_result:
            if number_equality_constraints:
                # second-order correction
                correction_old = concatenate_constraints(weights_0, slacks_0, equality_constraints, inequality_constraints, number_equality_constraints, number_inequality_constraints)
                correction_new = concatenate_constraints(weights_0 + alpha_smax * search_direction_weights, slacks_0, equality_constraints, inequality_constraints, number_equality_constraints, number_inequality_constraints)
                if np.sum(np.abs(correction_new)) > np.sum(np.abs(correction_old)):
                    # infeasibility has increased, attempt to correct
                    jacobian_matrix_objective_function = jacobian_objective_function(weights, number_weights, equality_constraints, inequality_constraints, number_equality_constraints, number_inequality_constraints)
                    try:
                        ...
                        # if the Jacobian is not invertible, find the minimum norm solution instead
                        feasibility_restoration_direction = -np.linalg.lstsq(jacobian_matrix_objective_function, correction_new, rcond=None)[0]
                    if merit_function(objective_function, weights_0 + alpha_smax * search_direction_weights + feasibility_restoration_direction, slacks_0, merit_function_parameter) <= merit_function_result + alpha_smax * armijo_val * gradient_merit_function_result:
                        # correction accepted
                        if verbosity > 2:
                            print('Second-order feasibility correction accepted')
            ...
            if not correction:
                ...
                alpha_smax *= backtracking_line_search_parameter
                alpha_lmax *= backtracking_line_search_parameter
                while merit_function(objective_function, weights_0 + alpha_smax * search_direction_weights, slacks_0, merit_function_parameter) > merit_function_result + alpha_smax * armijo_val * gradient_merit_function_result:
                    # backtracking line search
    ...
    # update weights
    if correction:
        weights = weights_0 + alpha_corr * (alpha_smax * search_direction_weights + feasibility_restoration_direction[:number_weights])
    else:
